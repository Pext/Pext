...
# Install Python
if [ "${UBUNTU_RELEASE}" = "ubuntu-22.04" ]; then
  sudo apt-get install -y python3.10 python3.10-venv python3.10-dev
elif [ "${UBUNTU_RELEASE}" = "ubuntu-20.04" ]; then
  sudo apt-get install -y python3.9 python3.9-venv python3.9-dev
elif [ "${UBUNTU_RELEASE}" = "ubuntu-18.04" ]; then
  sudo apt-get install -y python3.8 python3.8-venv python3.8-dev
else
  echo "ERROR: The Ubuntu version '${UBUNTU_RELEASE}' is outside the scope (ubuntu-18.04, ubuntu-20.04 or ubuntu-22.04)."
  lsb_release -a
  exit 1
fi
