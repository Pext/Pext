...
pushd "$AIUT_DIR"/
url=$(wget -qO- https://api.github.com/repos/AppImage/AppImageUpdate/releases | grep browser_download_url | cut -d: -f2- | sed 's|"||g' | grep appimageupdatetool | grep -E 'x86_64.AppImage$' | sed 's| ||g')
wget "$url"
